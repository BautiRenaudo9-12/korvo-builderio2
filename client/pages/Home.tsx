    ...
    <div className="px-6 md:px-8 pt-4 md:pt-8 animate-fade-in">
      ...
      <div className="glass-panel rounded-2xl p-6 md:p-8 relative overflow-hidden group mb-6 md:mb-8 shadow-2xl border border-border">
        ...
        <div className="flex flex-col items-center justify-center relative z-10">
          ...
          <p className="text-muted-foreground font-mono text-[10px] md:text-xs tracking-[0.2em] uppercase">ID: 884-291-KOR</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8 delay-100 animate-fade-in">
        <button
          onClick={goToWallet}
          className="glass-panel p-4 md:p-6 rounded-xl flex items-center gap-3 hover:bg-black/5 transition-colors text-left group border border-border md:col-span-2"
        >
          <div className="w-9 md:w-12 h-9 md:h-12 rounded-full bg-secondary flex items-center justify-center border border-border group-hover:border-primary/50 transition-colors flex-shrink-0">
            <Gift size={16} className="text-secondary-foreground md:hidden" />
            <Gift size={20} className="text-secondary-foreground hidden md:block" />
          </div>
          <div>
            <p className="text-xs md:text-sm font-semibold text-foreground">Canjear</p>
            <p className="text-[10px] md:text-xs text-muted-foreground">Ir a billetera</p>
          </div>
        </button>
        <button
        ...
        >
          <div className="w-9 md:w-12 h-9 md:h-12 rounded-full bg-secondary flex items-center justify-center border border-border group-hover:border-primary/50 transition-colors flex-shrink-0">
            <History size={16} className="text-secondary-foreground md:hidden" />
            <History size={20} className="text-secondary-foreground hidden md:block" />
          </div>
          <div>
            <p className="text-xs md:text-sm font-semibold text-foreground">Actividad</p>
            <p className="text-[10px] md:text-xs text-muted-foreground">Ver historial</p>
          </div>
        </button>
      </div>
